...
import { BoundingBox2, BoundingCircle } from 'gophergfx';
...
export class SpaceMinesweeper extends gfx.GfxApp
{
    ...
    update(deltaTime: number): void 
    {
        ...
        for(let i=0; i < this.starfield.numParticles; i++)
        {
            ...
            starPosition.x += this.mousePosition.x*(-0.05);
            starPosition.y += this.mousePosition.y*(-0.05);

           // ADD YOUR CODE HERE
            const starSize = this.starfield.particleSizes[i];
            starPosition.x += moveDirection.x*(starSize);
            starPosition.y += moveDirection.y*(starSize);
        ...
        this.mines.children.forEach((mine: gfx.Transform2) => {
            ...
            mine.position.x -= moveDirection.x * mineSpeed;
            mine.position.y -= moveDirection.y * mineSpeed;
        ...
        this.lasers.children.forEach((laser: gfx.Transform2) => {
            ...
            //create and update the direction of the laser
            const laserDirection = gfx.Vector2.normalize(laser.position);
            laser.position.x += laserDirection.x * laserSpeed;
            laser.position.y += laserDirection.y * laserSpeed;
            //remove the mine if it is hit by a laser
            if(laser.intersects(this.mine))
            {
                this.mine.remove();
            }
            //remove the laser if it is outside the scene
    ...
    onMouseDown(event: MouseEvent): void 
    {
        ...
        // ADD YOUR CODE HERE
        const laserSpawnDistance = 0.05;
        const laserIns = new gfx.ShapeInstance(this.laser);
        laserIns.lookAt(this.mousePosition);
        this.lasers.add(laserIns);
        laserIns.translateY(laserSpawnDistance);
    }
    ...
    private checkForMineCollisions(): void
    {
        for(let i=0; i < this.mines.children.length; i++)
        {
            for(let j=i+1; j < this.mines.children.length; j++)
            {
                ...
                // ADD YOUR CODE HERE
                //Check if the mines are colliding. If they are, make them explode.
                if(mine1.intersects(mine2))
                {
                    mine1.explode();
                    mine2.explode();
                }
                //Check if the ship is colliding with the mines. If it is, make them explode.
                if(mine1.intersects(this.ship))
                {
                    mine1.explode();
                }
                if(mine2.intersects(this.ship))
                {
                    mine2.explode();
                }
    ...
    private checkForLaserCollisions(): void
    {
        this.lasers.children.forEach((laser: gfx.Transform2)=>{
            this.mines.children.forEach((mineElem: gfx.Transform2)=>{
                ...
                // ADD YOUR CODE HERE
                //Check if the laser is colliding with the mine. 
                //If it is, make the mine explode and remove the laser.
